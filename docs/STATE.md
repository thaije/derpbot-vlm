...
- **Bluetooth enable at launch (#28)**: `RelayActivity` gates relay start on BT being on. If BT is off, it launches the system `ACTION_REQUEST_ENABLE` dialog (Android 13+ blocks `BluetoothAdapter.enable()` for non-privileged apps — logcat: "Caller does not match restriction criteria"). After the dialog, the service starts; if BT is still off, `RvrBleConnection` surfaces `State.UNAVAILABLE` → app-screen status line turns red + panel logs a `msg-warn`. The manual Connect button retries the BLE scan via `RvrRelayService.retryBleIfEnabled()` once the user enables BT. Camera-only mode skips the BT gate.
...
- **`decisions.jsonl` events**: `agent_start` (target, model, run_dir), `system_prompts` (detection + verifier system prompt text, logged once at start), `agent_stop`, `decision` (vis, heading, turn_angle_deg, dist, loc, confirmed, reason, prompt, vlm_latency_ms, frame), `verifier` (confirmed, matches, mismatches, reason, latency_ms), `vlm_error` (stage=decision|verifier|scan), `arrived`, `hazard` (kind, magnitude), `mode` (teleop bool), `toggle` (which, value), `set_target` (old, new, description), `estop`, `manual_query`, `ble_state` (state), `phone_connect`, `phone_disconnect`, `scan_start` (reason, steps, step_deg), `scan_decision` (step, vis, turn, dist, loc, reason, vlm_latency_ms, frame), `scan_end` (reason), `forced_drive` (dist, reason), `confirm_request` (target), `confirm_result` (confirmed, feedback).

### User confirmation (--confirm-with-user, on by default)
- When the verifier confirms a target sighting and the robot is within `arrive_dist_m`, the agent emits a `confirm_target` event to the panel with the detection frame + target name + verifier reason. The panel shows a modal dialog: "Is this the correct object?" with Yes/No buttons.
- Agent waits up to 120s for the user's response (`confirm_target_ack` message). Yes → `_on_arrived` (beep + switch to teleop). No → continue searching (resets loop counter, halts).
- `--no-confirm-user` CLI flag disables this (auto-arrive on verifier confirmation).
- **Rejected detections** are stored with the VLM's description of the object (reason text), its location, and the user's optional feedback. Both the detection prompt and the verifier prompt include a "Previously rejected by the user" section with these descriptions, so the VLM avoids re-flagging the same wrong object and the verifier won't re-confirm it.
